...
import { Link2Icon, ListCheckIcon, UploadIcon } from "lucide-react";
...
export default function TopicsTablePage() {
    ...
    const onSubmit = async (link: string) => {
        try {
            const res = await axios.post("/api/topics/notes", { link: link, topicId: topicId })
            if (res.status == 200) {
                toast.success("Notes Unloaded")
            }
        } catch (error) {
            console.log("Error for Upload Notes form Front Page: ", error)
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <button
                                                                    onClick={() => { setShow(true), setTopicId(topic.id), setTopicName(topic.title) }}
                                                                    className="p-2 text-gray-600 hover:text-green-600 hover:bg-red-50 rounded-lg transition-colors"
                                                                    title="Delete"
                                                                >
                                                                    <UploadIcon size={23} />
                                                                </button>
